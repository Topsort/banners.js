...
properties on the `<topsort-banner>` element, reflecting the `width` and `height`
attributes. You can use these in your stylesheet to size child elements, or override
them to make banners responsive.

The inner container uses the class `ts-banner`, making it easy to target with standard
CSS selectors.

```css
/* Override dimensions for responsive layouts */
topsort-banner {
  --ts-banner-width: 100%;
  --ts-banner-height: auto;
}
